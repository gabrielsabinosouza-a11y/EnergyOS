...
// POST /api/focus-rooms/[roomId]/stop — stop an active/paused room early (host
// only). Ends the session for EVERYONE; participants still focusing are marked
// as given up ("desistiu") and receive no completion rewards.
export async function POST(
  ...
  return handleRoute(async () => {
    ...
    const stopped = await stopFocusRoom(Number(roomId));
    return jsonOk({ room: stopped, message: "Sessão encerrada para todos." });
  });
}