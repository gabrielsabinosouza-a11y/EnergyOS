...
import { getProfile, upsertAndGetProfile, updateDisplayName } from "@/lib/db/profiles";
...
export async function PATCH(request: NextRequest) {
  return handleRoute(async () => {
    ...
    if (body.displayName === undefined) return jsonOk({ user: await getProfile(profileId) });
