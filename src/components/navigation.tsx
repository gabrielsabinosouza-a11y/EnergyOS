...
export function Sidebar({ pathname }: { pathname: string }) {
  return <aside className="hidden w-[252px] shrink-0 flex-col border-r border-white/8 px-6 py-8 lg:fixed lg:inset-y-0 lg:flex"><Link href="/" className="mb-16 flex items-center gap-3 px-2"><span className="brand-mark"><Sparkles size={17} /></span><span className="font-display text-xl font-semibold tracking-[-0.04em]">energy<span className="text-[#71d4ff]">OS</span></span></Link><nav className="space-y-1">{navigationItems.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={`nav-item ${pathname === href ? "active" : ""}`}><Icon size={17} /><span>{label}</span></Link>)}</nav></aside>;
}
